...
var GIFT = {
  ...
  to: "Ánh",
  date: "12/08/2026",
  title: "Cuộc chạy của Ánh",
  line: "Một game nhỏ thôi. Chơi thử nha.",
};
...
function getPlayCameraPosition() {
  var portrait = HEIGHT > WIDTH;
  var narrow = WIDTH < 768;
  return {
    x: 0,
    y: portrait ? 38 : 30,
    z: cameraPosGame + (portrait ? 55 : narrow ? 25 : 0),
    lookY: portrait ? 28 : 30,
  };
}
...
function applyResponsiveCamera() {
  ...
  if (gameStatus === "play") {
    ...
    camera.lookAt(new THREE.Vector3(0, pos.lookY, 0));
  }
...
function initScreenAnd3D() {
  ...
  fieldOfView = HEIGHT > WIDTH ? 68 : 50;
  nearPlane = 1;
  farPlane = 2000;
  ...
  // Cap pixel ratio for smoother play on high-DPI phones
  ...
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", handleWindowResize, false);
  }
...
function gameOver() {
  if (fieldGameOver) fieldGameOver.classList.add("show");
  gameStatus = "gameOver";
  monster.sit();
  hero.hang();
...
function replay() {
  ...
  if (fieldGameOver) fieldGameOver.classList.remove("show");
  ...
  var playCam = getPlayCameraPosition();
